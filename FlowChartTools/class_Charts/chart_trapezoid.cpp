#include "chart_trapezoid.h"


void Chart_Trapezoid::paintChart(QPainter & p)
{
    QPen tmp = p.pen();
    ...
    graphPath->moveTo(sx+((ex - sx)>>2),sy);
    graphPath->lineTo(ex-((ex - sx)>>2),sy);
    graphPath->lineTo(ex,ey);
    graphPath->lineTo(sx,ey);
    graphPath->lineTo(sx+((ex - sx)>>2),sy);
...
void Chart_Trapezoid::updateMagPointInfo()
{
    ...
    magPoint.i_point[0]->setX(x1 + ext);
